...
export class DjangoApiClient implements IApiClient {
  ...
  private getLocalizedCommonStrings() {
    let locale: Locale = 'en';
    if (typeof window !== 'undefined') {
      locale = (localStorage.getItem('conceptscroll-locale') as Locale) || 'en';
    }
    return getStrings(locale).common;
  }
  ...
  public async request<T>(
    ...
    const headers: HeadersInit = {
      ...
      // 'ngrok-skip-browser-warning': 'true', // Removed to avoid CORS preflight errors if backend doesn't allow this header
      ...options.headers,
    };
    ...
    try {
        ...
        if (refreshed) {
          ...
          if (!retryResponse.ok) {
            ...
            if (retryResponse.status === 401 || retryResponse.status === 403) {
                 console.log('🔄 Retry failed with 401/403, clearing auth and triggering logout');
                 this.clearAuth();
          ...
          console.log('🔄 Token refresh failed (returned false)');
          // Refresh failed, clear tokens
          this.clearAuth();
          if (this.unauthorizedCallback) {
            console.log('🔄 Triggering unauthorized callback');
            this.unauthorizedCallback();
          }
      ...
      if (response.status === 401 || response.status === 403) {
         console.log(`⛔ Request failed with ${response.status} (No refresh token or refresh loop), triggering logout`);
         this.clearAuth();
         if (this.unauthorizedCallback) {
           this.unauthorizedCallback();
         }
      ...
      if (!response.ok) {
        ...
        // Enhanced error parsing to handle various backend error formats including "error" field
        ...
        // Clean up stringified Python lists like "['Error message']"
        ...
        // If it's an actual array, join it
      ...
      const commonStrings = this.getLocalizedCommonStrings();
      ...
      if (error.message === 'Failed to fetch' || error.name === 'TypeError') {
        ...
        // Throw a localized user-friendly error
        throw new Error(commonStrings.errorNetworkFailed || 'Network connection failed');
      } else {
  ...
  async requestOTP(phoneNumber: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      console.error('Backend requestOTP failed:', error);
      throw error;
    }
  }
  ...
  async getUserProfile(): Promise<UserProfile> {
    const response = await this.request<any>(API_ENDPOINTS.USER_PROFILE);
    
    let profileData: any = {};
    
    // Check if we have the nested structure from the log: { success: true, data: { user: {...}, onboarding: {...} } }
    if (response.data && response.data.user) {
        // We have the nested user object
        profileData = {
            ...response.data.user,
            // If there are other fields in data.onboarding we might want, we could map them too
            // But UserProfile mainly cares about user fields
        };
    } else if (response.data && (response.success === true || response.status === 'success')) {
        // Standard wrapped response { success: true, data: { ...fields... } }
        profileData = response.data;
    } else if (response.data) {
        // Fallback for just { data: ... } wrapper
        profileData = response.data;
    } else {
        // Fallback if response itself is the profile
        profileData = response;
    }

    // Map backend fields to frontend interface expected fields
    return {
        ...profileData,
        // Backend returns IDs in board, medium, class_grade
        // Frontend often looks for *_id properties
        board_id: profileData.board_id || profileData.board,
        medium_id: profileData.medium_id || profileData.medium,
        class_id: profileData.class_id || profileData.class_grade || profileData.class,
        // Ensure original fields are also preserved/normalized
        board: profileData.board,
        medium: profileData.medium,
        class_grade: profileData.class_grade,
        // Ensure is_super_admin is mapped if present
        is_super_admin: profileData.is_super_admin,
    } as UserProfile;
  }

  async updateUserProfile(updates: Partial<UserProfile>): Promise<UserProfile> {
    const response = await this.request<any>(API_ENDPOINTS.USER_UPDATE_PROFILE, {
      method: 'PATCH',
  ...
  async getOnboardingPreferences(): Promise<OnboardingPreferences | null> {
    try {
      ...
      console.error('Failed to get onboarding preferences:', error);
  ...
  async getSubjects(classId: string, boardId?: string): Promise<Subject[]> {
    let url = `${API_ENDPOINTS.SUBJECTS}?class_id=${classId}`;
    if (boardId) {
  ...
  async getContentFeed(filters?: {
    ...
    const response = await this.request<{ results: any[]; count: number; next: string | null; previous: string | null }>(url);

    // Map raw items to ContentItems
    const mappedResults = (response.results || []).map(item => this.mapApiToContentItem(item));
  ...
  async getChapterNotes(chapterId: string): Promise<Note[]> {
    ...
    // Map ContentItem to Note and filter by type 'note' to ensure type safety
    return response.results
      .filter(item => item.type === 'note')
      .map(item => item.content as Note);
  }
  ...
  async getCurrentDailyGoal(): Promise<DailyGoal | null> {
    const response = await this.request<{ success: boolean; data: any }>(API_ENDPOINTS.DAILY_GOAL_CURRENT);
    ...
    if (response.success && response.data) {
        ...
        // 2. Incomplete (is_completed = false)
        // 3. First one
        const activeGoal = mappedGoals.find(g => g.is_active);
        if (activeGoal) return activeGoal;
        
        const incompleteGoal = mappedGoals.find(g => !g.is_completed);
        if (incompleteGoal) return incompleteGoal;
  ...
  async createDailyGoal(data: import('../../types').GoalCreationRequest): Promise<DailyGoal> {
    const response = await this.request<{ success: boolean; data: DailyGoal; message: string }>(
      API_ENDPOINTS.DAILY_GOALS, 
      {
        method: 'POST',
  ...
  async updateDailyGoalProgress(goalId: string, completedMinutes: number): Promise<DailyGoal> {
    const response = await this.request<{ success: boolean; data: DailyGoal }>(
        API_ENDPOINTS.DAILY_GOAL_UPDATE(goalId),
        {
            method: 'PATCH',
  ...
  async startDailyGoalSession(goalId: string): Promise<DailyGoal> {
    const response = await this.request<{ success: boolean; data: DailyGoal; message: string }>(
      `${API_ENDPOINTS.DAILY_GOAL_UPDATE(goalId)}start-session/`,
      {
        method: 'POST'
      }
    );
  ...
  async stopDailyGoalSession(goalId: string, secondsStudied: number): Promise<DailyGoal> {
    const response = await this.request<{ success: boolean; data: DailyGoal; message: string }>(
      `${API_ENDPOINTS.DAILY_GOAL_UPDATE(goalId)}stop-session/`,
      {
        method: 'POST',
  ...
  async completeDailyGoal(goalId: string): Promise<import('../../types').GoalCompletionResponse> {
      const response = await this.request<{ success: boolean; data: import('../../types').GoalCompletionResponse; message: string }>(
          API_ENDPOINTS.DAILY_GOAL_COMPLETE(goalId),
          {
              method: 'POST'
          }
      );
  ...
  async getSubscriptionPlans(language: string = 'en'): Promise<SubscriptionPlan[]> {
    const url = `${API_ENDPOINTS.PAYMENTS_PLANS}?language=${language}`;
    const response = await this.request<{ success: boolean; data: SubscriptionPlan[] }>(url);
    
    // Map localized fields to display fields
    const plans = (response.data || []).map(plan => ({
      ...plan,
      displayFeatures: plan.localized_features || plan.features,
      displayDescription: plan.localized_description || plan.description
    }));
  ...
  async getCurrentSubscription(): Promise<Subscription> {
    const response = await this.request<{ success: boolean; data: any }>(API_ENDPOINTS.PAYMENTS_MY_SUBSCRIPTION);
    const sub = response.data;
    
    if (!sub) {
      // Return a default "free" subscription structure or throw specific error
      // Ideally returning null, but the signature says Subscription. 
      // Let's return a dummy free subscription or null if the caller handles it.
      // But looking at types, Subscription probably requires some fields.
      // Let's throw for now so the caller (ProfileSettings) catches it and defaults to 'free'.
    ...
    return {
      ...sub,
      start_date: sub.started_at || sub.start_date,
      end_date: sub.expires_at || sub.end_date,
    } as Subscription;
  }

  async getQuotaStatus(): Promise<QuotaStatus> {
    const response = await this.request<{ success: boolean; data: QuotaStatus }>(API_ENDPOINTS.PAYMENTS_QUOTAS);
    return response.data;
  }

  async createPaymentOrder(planId: string): Promise<PaymentOrder> {
    const response = await this.request<{ success: boolean; data: PaymentOrder }>(API_ENDPOINTS.PAYMENTS_CREATE_ORDER, {
      method: 'POST',
  ...
  async verifyPayment(
    ...
    const response = await this.request<{ success: boolean; data: PaymentVerificationResponse }>(API_ENDPOINTS.PAYMENTS_VERIFY, {
      method: 'POST',
      body: JSON.stringify({
  ...
  async getPaymentHistory(): Promise<PaymentHistory[]> {
    const response = await this.request<{ success: boolean; data: PaymentHistory[] }>(API_ENDPOINTS.PAYMENTS_HISTORY);
    return response.data || [];
  }

  async getSubscriptionHistory(): Promise<Subscription[]> {
    const response = await this.request<{ success: boolean; data: Subscription[] }>(API_ENDPOINTS.SUBSCRIPTION_HISTORY);
    return response.data || [];
  }

  async cancelSubscription(reason?: string, feedback?: string): Promise<any> {
    const response = await this.request<any>(API_ENDPOINTS.SUBSCRIPTION_CANCEL, {
      method: 'POST',
  ...
  async triggerScraper(data: { class_name: string; subject_name: string; medium?: string }): Promise<{ success: boolean; task_id: string; error?: string }> {
    const response = await this.request<any>(API_ENDPOINTS.SCRAPER_TRIGGER, {
      method: 'POST',
      body: JSON.stringify(data)
    });
    return response;
  ...
  async getScraperTaskStatus(taskId: string): Promise<import('../../types').ScrapingTask> {
    return this.request(API_ENDPOINTS.SCRAPER_STATUS(taskId));
  }

  async getScraperSessions(): Promise<{ success: boolean; sessions: import('../../types').ScrapeSession[] }> {
    const response = await this.request<any>(API_ENDPOINTS.SCRAPER_SESSIONS);
    // Handle both wrapped and direct array
    if (response.sessions) return response;
    if (Array.isArray(response)) return { success: true, sessions: response };
    return { success: true, sessions: [] };
  }

  async getScraperSessionDetails(sessionId: string): Promise<{ success: boolean; session: import('../../types').ScrapeSession; data: import('../../types').ScraperPreviewData }> {
    return this.request(API_ENDPOINTS.SCRAPER_SESSION_DETAILS(sessionId));
  }

  async verifyScraperSession(sessionId: string): Promise<{ success: boolean }> {
    return this.request(API_ENDPOINTS.SCRAPER_SESSION_VERIFY(sessionId), {
      method: 'POST'
    });
  }

  async importScraperSession(sessionId: string, options?: any): Promise<{ success: boolean; task_id?: string; message?: string }> {
    return this.request(API_ENDPOINTS.SCRAPER_SESSION_IMPORT(sessionId), {
      method: 'POST',
      body: JSON.stringify(options || {})
    });
  }

  async getImportTaskStatus(taskId: string): Promise<any> {
    // Note: The documentation mentions /api/admin/scraper/import-status/{task_id}/
    // We need to add this endpoint to API_ENDPOINTS if not present, or construct it here.
    // Let's assume we add it to config/api.ts or use a string template here.
    return this.request(`/api/admin/scraper/import-status/${taskId}/`);
  }

  async deleteScraperSession(sessionId: string): Promise<{ success: boolean }> {
    return this.request(API_ENDPOINTS.SCRAPER_SESSION_DELETE(sessionId), {
      method: 'DELETE'
    });
  }

  async extractChapterNames(sessionId: string, bookId: string, medium: string): Promise<void> {
    await this.request(API_ENDPOINTS.SCRAPER_EXTRACT_NAMES(sessionId, bookId), {
      method: 'POST',
      body: JSON.stringify({ medium })
    });
  }

  async updateScrapedChapter(sessionId: string, chapterId: string, data: { name?: string; number?: number }): Promise<void> {
    await this.request(API_ENDPOINTS.SCRAPER_CHAPTER_UPDATE(sessionId, chapterId), {
      method: 'PATCH',
      body: JSON.stringify(data)
    });
  }
  ...
  async getPublicClasses(): Promise<Class[]> {
    try {
      // Try specific public endpoint first
      const response = await this.request<{ results: Class[] }>(API_ENDPOINTS.PUBLIC_CLASSES);
      return response.results || [];
    } catch (error) {
      console.warn('Public classes fetch failed, trying standard endpoint', error);
      // Fallback: Return empty or try to adapt
      return [];
    }
  }

  async getPublicSubjects(classId: string): Promise<Subject[]> {
    try {
      const url = `${API_ENDPOINTS.PUBLIC_SUBJECTS}?class_id=${classId}`;
      const response = await this.request<any>(url);
  ...
  async getPublicChapters(bookId: string): Promise<Chapter[]> {
    try {
      const url = `${API_ENDPOINTS.PUBLIC_CHAPTERS}?book_id=${bookId}`;
      const response = await this.request<any>(url);
